...
void Motor_Stop(void){
        P3OUT &=  ~RIGHT_MOT_SLEEP; //right motor sleep
        P3OUT &=  ~LEFT_MOT_SLEEP;  //left motor sleep
        TA0CCR3  = 0;      //stop PWM right side in Capture/compare unit 3
        TA0CCR4  = 0;      //stop PWM left side in Capture/compare unit 4
}
...
void Motor_Right(uint16_t rightDuty, uint16_t leftDuty){
        ...
        TA0R = 0;                  //counter, start at zero once turned on
        TA0CCR3  = 0;              //no PWM on right side
        TA0CCR4  = leftDuty;       //left side high time goes in Capture/compare unit 4
...
void Motor_Backward(uint16_t rightDuty, uint16_t leftDuty){
    //your code here
    // This function sets the motors to drive backwards with a PWM signal fixed at 10ms
       // having a duty cycle input when the function is called for each motor independently
