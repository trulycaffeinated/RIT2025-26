...
*Tangent - reg file is a file that holds all the registers used that are ported to the appropriate registers in the system. Any registers not used get ignored* 
**Internal Register** - Has access to it's own register file where we can store lots of data
...
44100/sec x 2 bytes (16 bits) x 2 channels (stereo) x 300sec
$$
44100*2*2*300 = 52920000 \text{ Bytes}
$$
We need compression - because that is a massive 52MB

SRAM exists on the board outside the FPGA. 
Nyquist is somewhat inaccurate - it is a bar minimum. (5x the rate might be better generally - according to jackson)
